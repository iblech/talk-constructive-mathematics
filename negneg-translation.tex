...
\frame{\frametitle{A classical logic fairy tale}
  ...
  $A$ classically \tabto{3.93cm}$\Longleftrightarrow$
...
\frame{\frametitle{Curry--Howard correspondence}
  ...
  \begin{tabular}{r@{\quad}l}
    ...
    \hil{$\inegneg$-translation} & \hil{CPS transform} \pause \\
    \only<3>{$\neg\neg A$ & ?? \\} \pause
...
\frame{\frametitle{Outlook}
  \begin{itemize}
    \item CPS transform $=$ Yoneda embedding
    \item Geometrical interpretation:
    \[ \Sh(X) \models A^\Box \qquad\Longleftrightarrow\qquad
    \Sh(X_{\neg\neg}) \models A \]
...
3. Relation to continuations
   ...
   * negneg-translation = CPS transform
