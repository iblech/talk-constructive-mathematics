...
%    \item Speziell folgt~$\neg(\neg B \wedge \neg \neg B)$. Es kann also auch
%          intuitionistisch für eine Aussage~$B$ nicht sein, dass weder sie noch
%          ihre Negation folgt.
%  \end{itemize}
%}
%
...
%      \neq \varnothing$, denn das ist gleichbedeutend mit~$\neg(\neg \exists x
...
%      \item Klassisch gilt~$\Omega = \{ \varnothing, \{ \star \} \}$, aber
...
%      ungleich~$\{ \star \}$ und ungleich~$\varnothing$ sind
%      (Abwärtskompatibilität zur klassischen Logik!).
%      \item \vspace{-0.5em} Es gilt $\Omega_{\text{entscheidbar}} = \{ \varnothing, \{ \star \} \}$ und
...
%    \item Sollte alle Teilmengen endlich indizierter Mengen wieder
%          endlich indiziert sein, gilt LEM.
%  \end{itemize}}
%}
%
%
...
%      nicht im Bild von~$f$ liegt. Somit gilt~$\varphi(\{y\}) = \varnothing =
%      \varphi(\varnothing)$ im Widerspruch zur Injektivität von~$\varphi$.
%    \end{proof}
...
\frame[t]{\frametitle{Kalkül natürlichen Schließens}
  \begin{itemize}
    ...
    \AxiomC{}\UnaryInfC{$A \seq{\vec x} A$}\DisplayProof
    \AxiomC{$A \seq{\vec x} B$}\UnaryInfC{$A[\vec s/\vec x]
    \seq{\vec y} B[\vec s/\vec x]$}\DisplayProof
    \AxiomC{$A \seq{\vec x} B$}\AxiomC{$B \seq{\vec x}
    C$}\BinaryInfC{$A \seq{\vec x} C$}\DisplayProof
  \end{itemize}
}
...
\frame[t]{\frametitle{Doppelnegationsübersetzung I}
  \begin{definition}
    \begin{itemize}
      \item
...
\frame[t]{\frametitle{Doppelnegationsübersetzung II}
  \begin{definition}
  ...
  \note{
    \begin{proof}[Beweis des Satzes]
      \begin{enumerate}
        ...
        Ableitungen im Gentzenkalkül). \qedhere
      \end{enumerate}
    \end{proof}
